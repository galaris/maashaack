/* -*- c-basic-offset: 4; indent-tabs-mode: nil; tab-width: 4 -*- */
/* vi: set ts=4 sw=4 expandtab: (add to ~/.vimrc: set modeline modelines=5) */
/* ***** BEGIN LICENSE BLOCK *****
 * Version: MPL 1.1/GPL 2.0/LGPL 2.1
 *
 * The contents of this file are subject to the Mozilla Public License Version
 * 1.1 (the "License"); you may not use this file except in compliance with
 * the License. You may obtain a copy of the License at
 * http://www.mozilla.org/MPL/
 *
 * Software distributed under the License is distributed on an "AS IS" basis,
 * WITHOUT WARRANTY OF ANY KIND, either express or implied. See the License
 * for the specific language governing rights and limitations under the
 * License.
 *
 * The Original Code is [Open Source Virtual Machine.].
 *
 * The Initial Developer of the Original Code is
 * Adobe System Incorporated.
 * Portions created by the Initial Developer are Copyright (C) 2004-2006
 * the Initial Developer. All Rights Reserved.
 *
 * Contributor(s):
 *   Adobe AS3 Team
 *   Zwetan Kjukov <zwetan@gmail.com>.
 *
 * Alternatively, the contents of this file may be used under the terms of
 * either the GNU General Public License Version 2 or later (the "GPL"), or
 * the GNU Lesser General Public License Version 2.1 or later (the "LGPL"),
 * in which case the provisions of the GPL or the LGPL are applicable instead
 * of those above. If you wish to allow use of your version of this file only
 * under the terms of either the GPL or the LGPL, and not to allow others to
 * use your version of this file under the terms of the MPL, indicate your
 * decision by deleting the provisions above and replace them with the notice
 * and other provisions required by the GPL or the LGPL. If you do not delete
 * the provisions above, a recipient may use your version of this file under
 * the terms of any one of the MPL, the GPL or the LGPL.
 *
 * ***** END LICENSE BLOCK ***** */

package flash.display
{
    import avmplus.flash_api;
    
    /**
     * A ShaderInput instance represents a single input image for a shader kernel.
     * 
     * @langversion ActionScript 3.0
     * @playerversion Flash Player 10
     * @playerversion AIR 1.5
     */
    [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
    public dynamic final class ShaderInput
    {
        private var _input:Object;
        private var _width:int;
        private var _height:int;
        
        private var _channels:int;
        private var _index:int;
        
        public function ShaderInput()
        {
            CFG::dbg{ trace( "new ShaderInput()" ); }
            super();
        }

        //public native function get input():Object;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function get input():Object
        {
            CFG::dbg{ trace( "ShaderInput.get input()" ); }
            return _input;
        }

        //public native function set input( value:Object ):void;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function set input( value:Object ):void
        {
            CFG::dbg{ trace( "ShaderInput.set input( " + value + " )" ); }
            _input = value;
        }

        //public native function get width():int;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function get width():int
        {
            CFG::dbg{ trace( "ShaderInput.get width()" ); }
            return _width;
        }

        //public native function set width( value:int ):void;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function set width( value:int ):void
        {
            CFG::dbg{ trace( "ShaderInput.set width( " + value + " )" ); }
            _width = value;
        }

        //public native function get height():int;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function get height():int
        {
            CFG::dbg{ trace( "ShaderInput.get height()" ); }
            return _height;
        }

        //public native function set height( value:int ):void;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function set height( value:int ):void
        {
            CFG::dbg{ trace( "ShaderInput.set height( " + value + " )" ); }
            _height = value;
        }

        //public native function get channels():int;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function get channels():int
        {
            CFG::dbg{ trace( "ShaderInput.get channels()" ); }
            return _channels;
        }

        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        flash_api function set channels( value:int ):void
        {
            CFG::dbg{ trace( "[flash_api] ShaderInput.set channels( " + value + " )" ); }
            _channels = value;
        }

        //public native function get index():int;
        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        public function get index():int
        {
            CFG::dbg{ trace( "ShaderInput.get index()" ); }
            return _index;
        }

        [API(CONFIG::FP_10_0,CONFIG::AIR_1_5)]
        flash_api function set channels( value:int ):void
        {
            CFG::dbg{ trace( "[flash_api] ShaderInput.set index( " + value + " )" ); }
            _index = value;
        }
    }
}
