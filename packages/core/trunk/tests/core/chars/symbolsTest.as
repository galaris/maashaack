...
package core.chars 
{
    ...
    {
        public function symbolsTest(name:String = "")
        {
            super(name);
        }
        ...
        {
            assertEquals( 33 , symbols.length , "length failed, must be 33 and is " + symbols.length ) ;
        }
        ...
        public function testSymbols():void
        {
            ...
            assertEquals( symbols.length , ref.length , "#1" ) ;
            
            var len:int = symbols.length ;
            
            for( var i:int ; i<len ; i++ )
            { 
                assertEquals( ref[i] , symbols[i] , "#2 failed at " + i ) ;
            }
        }
    }
}
